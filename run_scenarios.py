...
def run_scens(location=None, screen_scens=None, vaccine=True, # Input data
    ...
    for msim in all_msims:
        ...
        #df['new_vaccinated'] = msim.results['new_vaccinated'][:]
        #df['new_cancer_deaths'] = msim.results['new_cancer_deaths'][:]
...
if __name__ == '__main__':
    ...
    #Test more realistic scenarios
                # screen_scens[f'AVE_0.2,0.9, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.2,treat_prob=0.9)
                # screen_scens[f'AVE_0.2,0.8, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.2,treat_prob=0.8)
                # screen_scens[f'AVE_0.2,0.6, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.2, treat_prob=0.6)
                # screen_scens[f'AVE_0.2,0.4, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.2, treat_prob=0.4)
                # screen_scens[f'AVE_0.2,0.2, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.2, treat_prob=0.2)
                # screen_scens[f'AVE_0.4,0.9, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.4,treat_prob=0.9)
                # screen_scens[f'AVE_0.4,0.8, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.4,treat_prob=0.8)
                # screen_scens[f'AVE_0.4,0.6, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.4, treat_prob=0.6)
                # screen_scens[f'AVE_0.4,0.4, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.4, treat_prob=0.4)
                # screen_scens[f'AVE_0.4,0.2, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.4, treat_prob=0.2)
                # screen_scens[f'AVE_0.6,0.9, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.6,treat_prob=0.9)
                # screen_scens[f'AVE_0.6,0.8, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.6,treat_prob=0.8)
                # screen_scens[f'AVE_0.6,0.6, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.6, treat_prob=0.6)
                # screen_scens[f'AVE_0.6,0.4, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.6, treat_prob=0.4)
                # screen_scens[f'AVE_0.6,0.2, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.6, treat_prob=0.2)
                # screen_scens[f'AVE_0.7,0.9, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.7,treat_prob=0.9)
                # screen_scens[f'AVE_0.7,0.8, {int(sens * 100)}%/{int(spec * 100)}%'] = dict(primary=test_pos_vals,screen_coverage=0.7,treat_prob=0.8)
...
#Varied loss to follow up
            ...
            alldfs += alldf
            sc.saveobj(f'{ut.resfolder}/{location}_{filestem}_novaccine.obj', alldf)

        bigdf = pd.concat(alldfs)
        sc.saveobj(f'{ut.resfolder}/{filestem}_novaccine.obj', bigdf)
